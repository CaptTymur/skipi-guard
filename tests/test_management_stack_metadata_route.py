...
class ManagementStackMetadataRouteTests(unittest.TestCase):
    ...
    def init_repo(self, repo: Path) -> None:
        ...
        seed = {
            "dist/index.html": '<meta name="app-version" content="0.1.2">\n',
        ...
        for f in ['tests/management_plugin_isolation_harness.mjs', 'tests/management_presence_contract_harness.mjs']:
            seed[f] = "process.exit(0);\n"
        self.commit_updates(repo, "seed management Stage 4 fixture", seed)
    ...
    def run_guard(self, repo: Path, result_json: Path, *, task: str | None = None, auto_task: bool = False, run_harness: bool = False):
        command = [str(GUARD), "verify", "--home", "management", "--repo", str(repo), "--base", "HEAD~1", "--head", "HEAD", "--json", str(result_json)]
        if auto_task:
    ...
    def make_candidate(self, repo: Path, **kwargs) -> None:
        ...
        self.commit_updates(repo, "management Stage 4 stack metadata", updates, deleted=deleted)

    def test_exact_seven_auto_routes_stack_metadata(self) -> None:
        with tempfile.TemporaryDirectory(prefix="skipi-guard-management-stack-") as tmp:
            root = Path(tmp)
            repo = root / "repo"
            repo.mkdir()
            self.init_repo(repo)
        ...
        self.assertEqual(payload["task_rule"], "management Stage 4 stack-metadata routing")
        self.assertEqual(sorted(payload["changed_files"]), STACK_METADATA_FILES)

    def test_eighth_path_blocked(self) -> None:
        with tempfile.TemporaryDirectory(prefix="skipi-guard-management-eighth-") as tmp:
            root = Path(tmp)
            repo = root / "repo"
            repo.mkdir()
            self.init_repo(repo)
    ...
    def test_route_config_exact(self) -> None:
        ...
        stack_rule = rules["management Stage 4 stack-metadata routing"]
        ...
        self.assertEqual(names[:2], ["management_stack_build_metadata", "management_stack_verification_negative_control"])
        self.assertEqual(names, ['management_stack_build_metadata', 'management_stack_verification_negative_control', 'management_plugin_isolation', 'management_presence_contract'])
