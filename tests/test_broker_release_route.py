...
class BrokerReleaseRouteTests(unittest.TestCase):
    """Settings-wave 5x2: canonical version-bump routing for the broker home.

    Mirrors the seafarer precedent (skipi-guard#33): the exact
    version-carrying file set routes to the release task; anything else
    stays on the fail-closed default path.
    """
    ...
    def init_repo(self, repo: Path) -> None:
        ...
        self.commit_files(
            ...
            "seed broker version",
            {
                "dist/index.html": "APP_VERSION='0.1.151';\n",
    ...
    def canonical_bump(self) -> dict[str, str]:
        return {
            "dist/index.html": "APP_VERSION='0.1.152';\n",
    ...
    def test_auto_routes_exact_canonical_version_bump_to_release(self) -> None:
        with tempfile.TemporaryDirectory(prefix="skipi-guard-broker-release-route-") as tmp:
            root = Path(tmp)
            repo = root / "repo"
            repo.mkdir()
            self.init_repo(repo)
            self.commit_files(repo, "bump broker version", self.canonical_bump())
    ...
    def test_canonical_bump_plus_unrelated_source_stays_blocked(self) -> None:
        with tempfile.TemporaryDirectory(prefix="skipi-guard-broker-release-scope-") as tmp:
            root = Path(tmp)
            repo = root / "repo"
            repo.mkdir()
            self.init_repo(repo)
    ...
    def test_non_release_task_cannot_touch_release_sensitive_paths(self) -> None:
        with tempfile.TemporaryDirectory(prefix="skipi-guard-broker-non-release-") as tmp:
            root = Path(tmp)
            repo = root / "repo"
            repo.mkdir()
            self.init_repo(repo)
    ...
    def test_protected_path_requires_override_under_release_task(self) -> None:
        with tempfile.TemporaryDirectory(prefix="skipi-guard-broker-protected-") as tmp:
            root = Path(tmp)
            repo = root / "repo"
            repo.mkdir()
            self.init_repo(repo)
    ...
    def test_release_runs_the_full_plugin_host_harness_set(self) -> None:
        with CONFIG_PATH.open("r", encoding="utf-8") as handle:
            config = json.load(handle)
        plugin_host = {
        ...
        release = {
            (entry["name"], entry["command"])
            for entry in config["harness_commands"]["release"]
        }
        self.assertEqual(release, plugin_host)
